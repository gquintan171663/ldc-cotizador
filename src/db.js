...
async function insertChildren(versionId, state, sum){
  ...
  lins.forEach((lin, li)=>{ const {r, ek}=lineaMeta[li];
    (r.opciones||[]).forEach((o, oi)=>{ const pr=(o.precios&&o.precios[ek])||{};
      opRows.push({linea_id:lin.id,naviera:o.navScac||"",agente:o.agente||null,costo_base:parseFloat(pr.base)||0,profit:parseFloat(pr.profit)||0,transito_dias:parseInt(o.transito)||null,vig_hasta:o.vigHasta||null,sugerida:(r.elegida??0)===oi});
...
export async function loadVersion(versionId){
  ...
  const rutas=Object.values(rutasMap).map(rm=>{
    ...
    const ops=navSet.map(k=>{ const parts=k.split(SEP); const nav=parts[0], agente=parts[1]||""; const precios={}; let transito="", vigHasta=""; Object.entries(rm.equipos).forEach(([eq,l])=>{ const op=(opByLinea[l.id]||[]).find(o=>(o.naviera||"")===nav&&(o.agente||"")===agente); if(op){ precios[eq]={base:String(op.costo_base??""),profit:String(op.profit??"")}; if(op.transito_dias!=null) transito=String(op.transito_dias); if(op.vig_hasta) vigHasta=String(op.vig_hasta).slice(0,10); } }); return {navScac:nav,agente,transito,vigHasta,precios}; });
