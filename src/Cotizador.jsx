...
export function Cotizador({ loadId, onDirty }){
  ...
  const exportarXlsx=async()=>{ const falt=faltanPOLPOD(); if(falt.length){ alert("Faltan datos obligatorios en las rutas (POL, POD, naviera y modo si hay ciudad):\n\n• "+falt.join("\n• ")); return; } if(!confirmProfit()) return; try{ await exportarExcel(stCotiz()); }catch(ex){ alert("Error al exportar a Excel: "+ex.message); } };
    ...
    {started&&(<>
      ...
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginTop:14,padding:"6px 0"}}>
        ...
        <div style={{display:"flex",gap:10}}>
          ...
          <Btn kind="ghost" onClick={exportarXlsx}>Exportar a Excel</Btn>
