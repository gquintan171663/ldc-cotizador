...
import { C, F, EQUIPOS, EQUIPO_CATS, NAVIERAS, navName, CATALOG, COMMODITY_INDUSTRIAS, tx, scopeFull, serviceMode, transportMode, n, round10, adicPorCont, cargosBL, inclPorCont, inclBL, subjPorCont, subjBL, subjectTo, enPrecio, esSubjectTo, money, MONEDAS, optPuertos, optCiudades, puertoNombre, paisDe, paisOrigen, paisDestino, rutaPaisLabel, tlDe, tlLabel, TRADELANES, tradeLabel, rutaEnTradelane, opcionActivaEq, mejorOpcionEq, ordenOpciones, ordenRecargos, ovRazon, PLANTILLA_RECARGOS, parseTarifario, ordenarRutas, ESTADOS_MX, ESTADOS_TODOS, optEstados, abrevEstado, surAplican } from "./lib.js";
...
function SurchargeGrid({surs,onChange,catalog,dir,equipos,editable=true,onPropagar,agentes}){
  ...
  const add=()=>onChange([...rows,{c:"",d:"",monto:"",moneda:"USD",incluido:true,desplegar:true,pago:"prepaid",basis:"contenedor"}]);
  ...
  return (<div style={{border:"1px solid "+C.sep2,borderRadius:8,background:"#fff"}}>
    <table style={{width:"100%",borderCollapse:"collapse"}}>
      <thead><tr style={{background:C.soft,borderBottom:"1px solid "+C.sep2}}>
        ...
        <th style={{...th,width:"12%"}}>Base cobro</th><th style={{...th,width:"13%",textAlign:"center"}} title="¿Quién paga el cargo? LDC = lo paga LDC y va en la tarifa (suma al costo). Cliente = subject to, se paga aparte (al arribo si es en destino).">¿Quién paga?</th><th style={{...th,width:"8%",textAlign:"center"}}>Mostrar</th><th style={{...th,width:"10%"}} title="Prepaid/Collect: solo etiqueta de dónde se paga; no afecta el cálculo.">Pago</th>{dir==="I"&&<th style={{...th,width:"13%"}} title="Todas = aplica a esta naviera en cualquier canal. Un agente = agency surcharge, solo para esa combinación Agente-Naviera.">Agente</th>}<th style={{...th,width:"3%"}}></th></tr></thead>
      <tbody>
        {rows.length===0&&<tr><td colSpan={dir==="I"?10:9} style={{padding:10,textAlign:"center",color:C.label,fontSize:12}}>Sin recargos — agrega filas</td></tr>}
        ...
        <tr style={{borderBottom:openSize[i]?"none":"1px solid "+C.sep}}>
          ...
          <td style={{...td,textAlign:"center"}}><select value={r.incluido?"ldc":"cli"} onChange={e=>set(i,{incluido:e.target.value==="ldc"})} title="LDC = lo paga LDC y va en la tarifa (suma al costo). Cliente = subject to, se paga aparte (al arribo si es en destino)." style={{...cell,padding:"5px 4px",fontSize:11,fontWeight:"bold",color:r.incluido?C.slate:C.red}}><option value="ldc">Paga LDC</option><option value="cli">Paga cliente</option></select></td>
        ...
        {openSize[i]&&<tr style={{borderBottom:"1px solid "+C.sep,background:C.soft}}><td colSpan={dir==="I"?10:9} style={{padding:"6px 10px"}}>
    ...
    <div style={{padding:"7px 9px",borderTop:"1px solid "+C.sep2,background:C.soft,display:"flex",justifyContent:"space-between",alignItems:"center",flexWrap:"wrap",gap:8}}>
      ...
      <span style={{fontSize:11,color:C.label}}>
        <span style={{color:C.green,fontWeight:"bold"}}>Paga LDC (en tarifa, suma):</span> <b style={{color:C.slate}}>{money(inclPorCont(rows,e20))}</b>/20' · <b style={{color:C.slate}}>{money(inclPorCont(rows,e40))}</b>/40'{inclBL(rows)>0&&<span> · BL <b style={{color:C.slate}}>{money(inclBL(rows))}</b></span>}
        <span style={{margin:"0 8px",color:C.sep2}}>|</span>
        <span style={{color:C.red,fontWeight:"bold"}}>Paga cliente (subject to):</span> <b style={{color:C.slate}}>{money(subjPorCont(rows,e20))}</b>/20' · <b style={{color:C.slate}}>{money(subjPorCont(rows,e40))}</b>/40'{subjBL(rows)>0&&<span> · BL <b style={{color:C.slate}}>{money(subjBL(rows))}</b></span>}
      </span>
    </div>
  </div>);
}
          ...
          return ordenOpciones(r,eqs,dir,surOf).map((oi,pos)=>{const o=r.opciones[oi];const surs=surAplican(surOf(o.navScac,tlDe(r)),o.agente),st=surs.filter(s=>esSubjectTo(s,dir)&&s.desplegar!==false).map(s=>s.c),bl=cargosBL(surs,dir),first=pos===0;
            return (<tr key={ri+"-"+oi} id={(first&&_anc)?eidTarifa(_tl):undefined} style={{background:(first&&hlRuta===ri)?"#FFF6D6":(first?"#fff":C.soft),boxShadow:(first&&hlRuta===ri)?"inset 3px 0 0 "+C.red:undefined,transition:"background .3s"}}>
              ...
              {eqs.map(e=>{const p=getP(o,e.k);const base=n(p.base),prof=n(p.profit);const adic=adicPorCont(surs,e,dir);const venta=base+adic+prof;const _best=mejorOpcionEq(r,e.k,e,dir,surOf);const _act=opcionActivaEq(r,e.k,e,dir,surOf)===oi;const _isBest=_act&&oi===_best;const _razon=ovRazon(r.elegidaEq&&r.elegidaEq[e.k]);
                const _summ=surs.filter(s=>s.incluido&&(s.basis||"contenedor")!=="bl");
...
export function Cotizador({ loadId, onDirty, role }){
  ...
  const importarTodo=()=>{
    ...
    grupos.forEach(([gr,lista])=>{ (lista||[]).forEach((m,mi)=>{ const gm=gr+"-"+mi; (m.navieras||[]).forEach((nv,ni)=>{ if(!nv.scac) return;
      Object.keys(nv.precios||{}).forEach(ek=>{ const p=nv.precios[ek]||{}; if(p.base==null||p.base===""||n(p.base)<=0) return; const k=gm+"|"+ni+"|"+ek; if(!(sim.sel&&sim.sel[k])) return; const dk=nv.scac+"|"+ek; if(yaImportado[dk]) return; yaImportado[dk]=true;
        let op=nuevasOps.find(o=>o.navScac===nv.scac); if(!op){ op={navScac:nv.scac,transito:nv.transito||"",precios:{}}; nuevasOps.push(op); addedNav++; }
        op.precios={...(op.precios||{}),[ek]:JSON.parse(JSON.stringify(p))}; addedEq++;
        let blk=nuevoQN.find(q=>q.scac===nv.scac&&(q.tl||"")===(tlDest||"")); if(!blk){ blk={scac:nv.scac,tl:tlDest||"",surcharges:[]}; nuevoQN.push(blk); }
        (nv.recargos||[]).forEach(sr=>{ if(!blk.surcharges.some(x=>String(x.c||"").toUpperCase()===String(sr.c||"").toUpperCase())) blk.surcharges.push({...sr}); });
      });
    }); }); });
    if(!addedEq){ alert("No marcaste ninguna tarifa para importar."); return; }
    setQuoteNav(nuevoQN);
  ...
  // Subir plantilla (inteligente): si el archivo trae la clave oculta __k (=> "Borrador a plantilla"),
  ...
  return (<div style={{maxWidth:1160,margin:"0 auto"}}>
    ...
    {started&&(<>
      ...
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:8,flexWrap:"wrap",gap:8}}>
        ...
        <div style={{display:"flex",gap:8,alignItems:"center"}}><input type="file" ref={impInputRef} accept=".xlsx,.xls" style={{display:"none"}} onChange={onPlantillaFile}/><Btn kind="ghost" small onClick={bajarPlantillaTarifario} title="Descarga una plantilla EN BLANCO para capturar tarifas nuevas">⬇ Plantilla vacía</Btn><Btn kind="ghost" small onClick={bajarBorradorExcel} title="Baja el borrador actual en formato plantilla (prellenado) para editarlo en Excel">⬇ Borrador a plantilla</Btn><Btn kind="ghost" small disabled={!editable} onClick={()=>impInputRef.current&&impInputRef.current.click()} title="Sube una plantilla. Si es un 'Borrador a plantilla' (trae su clave oculta), ACTUALIZA en su lugar conservando profit/recargos; si es una plantilla nueva, AGREGA o REEMPLAZA rutas.">⇪ Subir plantilla</Btn><Btn kind="ghost" small onClick={()=>setEditRutas(!editRutas)}>{editRutas?"Ocultar rutas":"Editar rutas"}</Btn><Btn kind="ghost" small disabled={!editable} onClick={()=>setRutas(ordenarRutas(rutas,direccion))} title="Ordenar por ciudad origen · país POL · región POD · país POD">↕ Ordenar rutas</Btn><Btn kind="ghost" small disabled={!editable} onClick={()=>setRutas([...rutas,mkRuta()])}>＋ Agregar ruta</Btn></div>
      </div>
      <fieldset disabled={!editable} style={{border:"none",padding:0,margin:0,minWidth:0,opacity:editable?1:.75}}>
      ...
      {sim&&<div style={{background:"#fff",border:"2px solid #1F6FB2",borderRadius:12,padding:16,marginTop:12}}>
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:4}}>
          <div style={{display:"flex",alignItems:"baseline",gap:10,flexWrap:"wrap"}}>
            ...
            {(()=>{ const r=rutas[sim.ri]||{}; const oc=r.origen||"", dc=r.destino||"", om=r.precarriage_mode||"", dm=r.oncarriage_mode||"", oe=r.origenEstado||"", de=r.destinoEstado||""; const _lc=(v)=>{ const nm=puertoNombre(v)||String(v||"?"); const pais=paisDe(v); return nm+(pais&&!/\(\s*[A-Z]{2}\s*\)\s*$/.test(nm)?" ("+pais+")":""); }; const lbl=(oc?(oc+(oe?", "+abrevEstado(oe):"")+(om?" ["+om+"]":"")+" › "):"")+_lc(r.pol)+" → "+_lc(r.pod)+(dc?(" › "+(dm?"["+dm+"] ":"")+dc+(de?", "+abrevEstado(de):"")):"")+(comLabel?" · "+comLabel:""); return <div onClick={()=>irARuta(sim.ri)} title="Ir a esta ruta en Tarifas" style={{fontSize:11.5,fontWeight:"bold",color:"#1F6FB2",background:"#E7F1FB",border:"1px solid #C3DCF2",borderRadius:6,padding:"3px 9px",cursor:"pointer"}}>{lbl} ↗</div>; })()}
          </div>
          <span onClick={()=>setSim(null)} style={{cursor:"pointer",color:C.label,fontSize:18,lineHeight:1}}>✕</span>
        </div>
        <div style={{fontSize:11,color:C.label,marginBottom:12,lineHeight:1.45}}>Marca los equipos a importar y usa <b>Importar seleccionadas</b> (un solo clic). No se puede marcar el mismo equipo+naviera dos veces. Clic en la ruta azul para volver a ella en Tarifas.</div>
          ...
          const tabla=(filas,conCheck,clickEq)=> (<table style={{width:"100%",borderCollapse:"collapse",fontSize:11.5,marginTop:4}}>
            ...
            <tbody>{filas.map((row,idx)=>{ const {k,ek,eqT,scac,base,rec,recN,prof,venta,dup}=row; const on=conCheck&&!!(sim.sel&&sim.sel[k]); const act=clickEq&&sim.filtroEq===ek; return (
              <tr key={idx} onClick={clickEq?()=>clickEq(ek):undefined} title={clickEq?"Clic para ver solo "+eqT+" abajo":undefined} style={{borderTop:"1px solid "+C.sep,textAlign:"right",opacity:conCheck?(dup?0.35:(on?1:0.55)):1,cursor:clickEq?"pointer":undefined,background:act?"#E7F1FB":undefined}}>
                ...
                <td style={{padding:"3px 6px",color:prof<250?C.red:C.slate}}>${prof.toLocaleString()}</td>
                <td style={{padding:"3px 6px",fontWeight:"bold",color:C.ink}}>${venta.toLocaleString()}</td>
              </tr>
            ); })}</tbody>
          </table>);
          ...
          const render=(lista,grupo,titulo,badge)=> (lista&&lista.length>0)&&(<div style={{marginBottom:16}}>
            ...
            {lista.map((m,mi)=>{ const dirRef=m.direccion||dirActual; const gm=grupo+"-"+mi; const pt=tagPuerto(m.puertoTag); const filas=[]; (m.navieras||[]).forEach((nv,ni)=>{ filaEq(nv,dirRef).forEach(ff=>{ const k=selKey(gm,ni,ff.ek); const dk=(nv.scac||"")+"|"+ff.ek; const dup=claimed[dk]&&claimed[dk]!==k; filas.push({k,eqT:ff.eqT,scac:nv.scac,base:ff.base,rec:ff.rec,recN:ff.recN,prof:ff.prof,venta:ff.venta,dup}); }); }); if(!filas.length) return null; const selKeys=filas.filter(x=>!x.dup).map(x=>x.k); return (<div key={mi} style={{border:"1px solid "+C.sep2,borderRadius:8,padding:"9px 11px",marginBottom:10}}>
              ...
              <div style={{display:"flex",justifyContent:"flex-end",marginTop:6}}>
                <span onClick={()=>{ const all=selKeys.length&&selKeys.every(k=>sim.sel&&sim.sel[k]); setSim(s2=>{ const nx={...(s2.sel||{})}; selKeys.forEach(k=>nx[k]=!all); return {...s2,sel:nx}; }); }} style={{fontSize:11,color:C.red,cursor:"pointer"}}>{selKeys.length&&selKeys.every(k=>sim.sel&&sim.sel[k])?"Quitar todos":"Seleccionar disponibles"}</span>
              </div>
            </div>); })}
          </div>);
