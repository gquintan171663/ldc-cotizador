...
import { C, F, EQUIPOS, EQUIPO_CATS, NAVIERAS, navName, CATALOG, COMMODITY_INDUSTRIAS, tx, scopeFull, serviceMode, transportMode, n, adicPorCont, cargosBL, inclPorCont, inclBL, subjectTo, enPrecio, esSubjectTo, money, MONEDAS, optPuertos, optCiudades, paisOrigen, paisDestino, rutaPaisLabel, tlDe, tlLabel, TRADELANES, tradeLabel, rutaEnTradelane, opcionActivaEq, mejorOpcionEq, ovRazon, PLANTILLA_RECARGOS, parseTarifario } from "./lib.js";
...
export function Cotizador({ loadId, onDirty }){
  ...
  const derivarAnclaje=(rts)=>(rts||[]).map(r=>{ if(!r.ventaAncla) return r; return {...r,opciones:(r.opciones||[]).map(o=>{ const precios={...(o.precios||{})}; Object.keys(r.ventaAncla).forEach(ek=>{ const eqObj=EQUIPOS.find(x=>x.k===ek); const pr=precios[ek]; if(!eqObj||!pr) return; const base=n(pr.base); if(pr.base==null||pr.base==="") return; const adic=adicPorCont(surOfMain(o.navScac,tlDe(r)),eqObj,direccion); precios[ek]={...pr,profit:String(Math.round(Number(r.ventaAncla[ek])-base-adic))}; }); return {...o,precios}; })}; });
