  ...
  return (<div style={{maxWidth:1160,margin:"0 auto"}}>
    ...
    {started&&(<>
      ...
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:8,flexWrap:"wrap",gap:8}}>
        ...
        <div style={{display:"flex",gap:8,alignItems:"center"}}><input type="file" ref={impInputRef} accept=".xlsx,.xls" style={{display:"none"}} onChange={onTarifarioFile}/><Btn kind="ghost" small onClick={bajarPlantillaTarifario} title="Descarga una plantilla EN BLANCO para capturar tarifas nuevas">⬇ Plantilla vacía</Btn><Btn kind="ghost" small onClick={bajarBorradorExcel} title="Baja el borrador actual en formato plantilla (prellenado) para editarlo en Excel">⬇ Borrador a plantilla</Btn><Btn kind="ghost" small disabled={!editable} onClick={()=>impInputRef.current&&impInputRef.current.click()} title="Sube una plantilla para AGREGAR o REEMPLAZAR rutas (tarifas nuevas)">⇪ Importar plantilla</Btn><input type="file" ref={baseInputRef} accept=".xlsx,.xls" style={{display:"none"}} onChange={onBaseFile}/><Btn kind="ghost" small disabled={!editable} onClick={()=>baseInputRef.current&&baseInputRef.current.click()} title="Sube el borrador editado (con su clave oculta) para ACTUALIZAR las tarifas base en su lugar; conserva profit/recargos/venta">⇪ Actualizar desde plantilla</Btn><Btn kind="ghost" small onClick={()=>setEditRutas(!editRutas)}>{editRutas?"Ocultar rutas":"Editar rutas"}</Btn><Btn kind="ghost" small disabled={!editable} onClick={()=>setRutas(ordenarRutas(rutas,direccion))} title="Ordenar por ciudad origen · país POL · región POD · país POD">↕ Ordenar rutas</Btn><Btn kind="ghost" small disabled={!editable} onClick={()=>setRutas([...rutas,mkRuta()])}>＋ Agregar ruta</Btn></div>
      </div>
      <fieldset disabled={!editable} style={{border:"none",padding:0,margin:0,minWidth:0,opacity:editable?1:.75}}>
