    ...
    {rows===null?<div style={{color:C.label,fontSize:13,padding:20}}>Cargando…</div>:(
      <div style={{border:"1px solid "+C.sep2,borderRadius:10,overflow:"auto",background:"#fff"}}>
        <table style={{width:"100%",borderCollapse:"collapse"}}>
          <thead><tr style={{background:C.ink}}>{["Folio","Cliente","Dir.","Vigencia","Estatus","Responsable","Actualizado",""].map((h,i)=><th key={i} style={th}>{h}</th>)}</tr></thead>
          <tbody>
            {filtered.length===0&&<tr><td colSpan={8} style={{...td,textAlign:"center",color:C.label,padding:24}}>Sin cotizaciones todavía.</td></tr>}
            {filtered.map(r=>(<tr key={r.id} style={{background:r.estatus==="superseded"?C.soft:"#fff"}}>
              ...
              <td style={{...td,fontSize:11,color:C.slate,whiteSpace:"nowrap"}}>{(r.vigDesde||r.vigHasta)?(fecha(r.vigDesde)+(r.vigHasta?" — "+fecha(r.vigHasta):"")):<span style={{color:C.label}}>—</span>}</td>
